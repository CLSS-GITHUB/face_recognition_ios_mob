...
class _LiveEnrollmentScreenState
    extends ConsumerState<LiveEnrollmentScreen> {
  ...
  String _bodyHint(EnrollmentStage stage) {
    return switch (stage) {
      ...
      EnrollmentStage.verify =>
        'One more blink to confirm enrollment.',
  ...
  void _reactToState(EnrollmentState? prev, EnrollmentState next) {
    final justEnteredRegistration = prev?.stage != EnrollmentStage.registration &&
        next.stage == EnrollmentStage.registration;
    final justFailedVerification = (prev?.showVerificationFailed ?? false) == false &&
        ...
        showDialog(
          ...
          builder: (_) => RegistrationDialog(
            ...
            onConfirm: (code, name) async {
              ...
              final result =
                  await controller.register(userCode: code, userName: name);
              if (!mounted) return;
        ...
        showDialog(
          ...
          builder: (_) => VerificationFailedDialog(
            onRetryVerification: () {
              ...
              ref.read(enrollmentControllerProvider.notifier).retryVerification();
            },
            onRestartEnrollment: () {
              Navigator.of(context).pop();
              _verificationFailedDialogOpen = false;
              ref.read(enrollmentControllerProvider.notifier).restartFromVerification();
            },
          ),
        );
      });
  ...
  void _showResultSnackBar(EnrollmentResult result) {
    ...
    final text = switch (result) {
      ...
      TemplateAddedToExisting(:final user) =>
        'Added template to ${user.name}.',
